...
/**
 * Callback block signature for operations on <tt>CMWebService</tt> that upload binary files to
 * the CloudMine servers without returning the new file key. These blocks return <tt>void</tt> 
 * and take a <tt>CMFileUploadResult</tt> as an argument to indicate the final result of the upload operation.
 */
typedef void (^CMWebServiceFileUploadSuccessCallback)(CMFileUploadResult result);

/**
 * Callback block signature for operations on <tt>CMWebService</tt> that upload binary files to
 * the CloudMine servers, returning the new file key generated by the server. These blocks return <tt>void</tt>, 
